...
def calculate_message_cost(usage: dict, pricing: dict[str, float]) -> float:
    ...
    Token breakdown from Anthropic API:
    ...
    NOTE: This is an APPROXIMATE calculation. Claude Code's internal cost
    calculation is not publicly documented. We reverse-engineered the cache
    read rate (~1.7% of input price) to match CLI values. Anthropic's
    documented rate is 10%, but that doesn't match what the CLI shows.
    See: https://github.com/quickcall-dev/quickcall-supertrace/issues/90
    """
    ...
    # Cache read - HACKY: using ~1.7% to match Claude Code CLI (not the documented 10%)
    cache_read_cost = (cache_read / 1_000_000) * pricing["input"] * 0.017
    # Cache write at 125% of input price (25% premium)
...
def handle_stop(hook_input: HookInput) -> None:
    ...
    debug(f"Usage: {total_context}/{context_size} tokens ({used_pct:.1f}%), cost: ${cost_usd:.2f}")
